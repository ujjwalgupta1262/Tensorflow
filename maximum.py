...
def generate_data(seq_length,training_set_size):
	...
	while(training_set_size > 0):
		...
		for i in range(seq_length):
			s += str(int(math.floor(10*random.random())))
		temp.append(s)
		training_set_size -= 1
	input_list = []
	label_list = []
...
output,state = tf.nn.static_rnn(hidden_layer,input,dtype = "float32")
...
with tf.Session() as sess:
	...
	#training
	while(iterations > 0):
		train_x,train_y = generate_data(10,100)
