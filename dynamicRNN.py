...
output,state = tf.nn.dynamic_rnn(hidden_layer,X,dtype = "float32")

#prediction
output = tf.transpose(output,[1,0,2])
last_output = tf.gather(output,int(output.get_shape()[0]) - 1)
pred = tf.matmul(last_output,output_weights) + output_bias
real_pred = tf.nn.softmax(pred);

#cost function
cost = -tf.reduce_sum(Y * tf.log(tf.clip_by_value(real_pred,1e-10,1.0)))
...
with tf.Session() as sess:
	...
	#training
	while(iterations > 0):
		train_x,train_y = generate_data(10,20)
	...
	prediction = sess.run(real_pred,feed_dict = {X : test_x, Y : test_y})
