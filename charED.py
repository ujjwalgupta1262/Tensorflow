...
with open(filename,"r") as fopen:
	...
	for line in total_lines[0:training_set_size]:
		...
		target_word = start_char + input_word + end_char
...
for i in xrange(len(input_words)):
	for j in xrange(len(input_words[i])):
		...
		encoder_inputs[i,j,encoder_tokens_ind[input_words[i][j]]] = 1 
	for j in xrange(len(target_words[i])):
		decoder_inputs[i,j,decoder_tokens_ind[target_words[i][j]]] = 1
		if(j > 0):
			decoder_outputs[i,j-1,decoder_tokens_ind[target_words[i][j]]] = 1
...
def get_sentence(encoder_input_seq):
	...
	while True:
		...
		#if output character is end character 
